...
class ActivationPatcher:
    ...
    def __init__(self, model, tokenizer, device="cuda", max_new_tokens=128, use_cache=True):
        self.model              = model
        self.tokenizer          = tokenizer
        self.device             = device
        ...
        self._noisy_prompt_len  = 0   # set before each generation when use_cache=False
    ...
    def _register_patch_hook(self, layer_idx: int, mode: str):
        def patch_hook(module, inp, out):
            ...
            # (h.shape[1] == 1). Patching is only meaningful during prefill.
            if self.use_cache and h.shape[1] <= 1:
                return out

            if mode in ("clean", "identity") and clean is not None:
                patched = h.clone()

                if not self.use_cache and self._noisy_prompt_len > 0 \
                        and h.shape[1] > self._noisy_prompt_len:
                    # Decode step with no cache: h = [prompt_tokens + generated_tokens].
                    # Only patch the prompt slice — leave generated positions untouched
                    # so their hidden states are not corrupted by clean-prompt states.
                    plen = self._noisy_prompt_len
                    h_prompt = h[:, :plen, :]
                    ml = min(plen, clean.shape[1])
                    h_prompt = h_prompt.clone()
                    h_prompt[:, plen - ml:, :] = clean[:, clean.shape[1] - ml:, :]
                    patched[:, :plen, :] = h_prompt
                else:
                    # Prefill (both use_cache modes) or full-sequence decode step.
                    # Suffix-align: patch the LAST ml positions so the
                    # assistant-start token is always included.
    ...
    def run_identity_all_layers(self, clean_prompt: str, noisy_prompt: str,
        ...
        if not self.use_cache:
            noisy_inp = self.tokenizer(noisy_prompt, return_tensors="pt",
                                       truncation=True, max_length=1024)
            self._noisy_prompt_len = noisy_inp["input_ids"].shape[1]
